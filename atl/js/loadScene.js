require([
	...
	*/
	function showFallback(errorObject) {
		document.getElementById('canvas-outer').style.display = 'none';
	...
	function init() {
		...
		document.getElementById('goo-loading-overlay').className = 'loading';
		...
		loadProject(gooRunner).then(function (loader) {
			...
			document.getElementById('goo-loading-overlay').className = '';
			CanvasWrapper.show();
